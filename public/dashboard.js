...
async function createChart() {
  try {
    ...
    const labels = data.map((item) => {
      ...
      // Ajustar para UTC-3 (horário de São Paulo)
    ...
    // Configurar tamanho fixo para o canvas
    canvas.style.height = '400px'; // Altura fixa para o canvas
    canvas.style.width = '100%'; // Largura máxima do container
    ...
    window.vpdChartInstance = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false, // Garantir que o gráfico seja responsivo sem crescer
        plugins: {
          title: {
            display: true,
